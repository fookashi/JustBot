...
class FrogTextConstructor:
    def __init__(self) -> None:
        self.NOT_WEDNSDAYS_DAY = (
            "похуй, главное, что не среда",
            "точно не среда",
            "не среда",
            "не не не среда",
            "не лягуший день",
        ...
        self.WEDNSDAYS_DAY = (
            "СВЯЩЕННЫЙ ДЕНЬ",
            "ЕБУЧАЯ МАТЬ ЕГО СРЕДА",
            "МОЙ ЛЮБИМЫЙ ДЕНЬ НЕДЕЛИ",
            "КВА-КВА-КВА ДЕНЬ",
            "ТОТ САМЫЙ ДЕНЬ",
            "просто среда, ладно я шучу, АХУННАЯ МАТЬ ЕГО СРЕДА",
            "среда моя среда",
            "хороший день, то есть среда",
            "среда",
            "ДЕНЬ X",
        )
        self.NOT_WEDNSDAYS_ENDINGS = (
            "Может следующий день будет особенным?(например средой)",
            "Ну ничего, среда уже скоро",
            "Не ничего, и на нашей улице будет среда",
            "Хорошо, что среда уже скоро",
        )
        self.WEDNSDAYS_ENDINGS = (
            "Из плохих новостей: следующая среда будет только через неделю",
            "Ладно, это просто обычный день недели, КОТОРЫЙ ПИЗЖЕ В СТО РАЗ ЛЮБОГО ДРУГОГО ДНЯ",
            "Помолимся же жабьему богу за этот чудесный день",
            "Какой же это кааааайф",
        )

    def create_text(self, weekday_num: int) -> str:
        weekday = weekday_to_string(weekday_num)
        if weekday_num != "wednsday":
            weekday_text = random.choices((weekday, random.choice(self.NOT_WEDNSDAYS_DAY)), (0.4, 0.6), k=1)[0]  # noqa: S311
            weekday_ending = random.choices(("", random.choice(self.NOT_WEDNSDAYS_ENDINGS)), (0.4, 0.6), k=1)[0]  # noqa: S311
        else:
            weekday_text = random.choices((weekday, random.choice(self.WEDNSDAYS_DAY)), (0.4, 0.6), k=1)[0]  # noqa: S311
            weekday_ending = random.choices(("", random.choice(self.WEDNSDAYS_ENDINGS)), (0.4, 0.6), k=1)[0]  # noqa: S311
